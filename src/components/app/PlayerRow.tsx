...
export function PlayerRow({ user, meta, actions, link = true }: { user: PublicUser; meta?: ReactNode; actions?: ReactNode; link?: boolean }) {
  const canLink = link && !user.isGuest;
  const body = (
    <>
      ...
      <div className="min-w-0 flex-1">
        ...
        {user.isGuest ? (
          <p className="truncate text-sm text-ink-muted">Avulso, sem conta</p>
        ) : (
  ...
  return (
    <div className="flex items-center gap-3 py-2.5">
      {canLink ? <Link to={`/jogador/${user.username}`} className="flex min-w-0 flex-1 items-center gap-3">{body}</Link> : <div className="flex min-w-0 flex-1 items-center gap-3">{body}</div>}
